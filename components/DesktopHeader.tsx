...
export function DesktopHeader({ authenticated = false, showNavigation = false, activeTab = "home" }: { authenticated?: boolean; showNavigation?: boolean; activeTab?: string }) {
  ...
  return (
    <header className="relative hidden h-[125px] w-full shrink-0 grid-cols-3 items-center px-[50px] md:grid xl:px-[120px]">
      <BrandLogo />
      <div className="flex justify-center">
        {showNavigation && (
          <nav aria-label="주요 메뉴" className="flex items-center gap-[14px] rounded-[30px] border-[1.4px] border-[#e2dafa] bg-[#fbfafd] py-2 pl-3 pr-2">
            {tabs.map((tab) => {
      ...
      <div className="flex items-center justify-end gap-[30px]">
        <div className="flex items-center gap-3.5"><div className="size-[43px] rounded-full bg-[#f3f3ff]" /><div className="flex flex-col gap-1"><strong className="text-lg font-bold leading-none text-[#43306d]">{authenticated ? user.name : "비회원"}</strong><span className="text-[15px] font-medium leading-none text-[#796b6c]">{authenticated ? user.email : "로그인해 주세요"}</span></div></div>
