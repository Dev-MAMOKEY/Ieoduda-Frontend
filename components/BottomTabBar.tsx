...
export function BottomTabBar({ activeTab, className = "" }: { activeTab: string; className?: string }) {
  return (
    <nav aria-label="주요 메뉴" className={`fixed bottom-[max(20px,env(safe-area-inset-bottom))] left-1/2 z-50 flex min-h-[60px] -translate-x-1/2 items-center gap-[14px] rounded-[30px] border-[1.4px] border-[#e2dafa] bg-[#fbfafd]/95 py-2 pl-3 pr-2 shadow-[0_4px_16px_rgba(67,48,109,0.12)] backdrop-blur-sm lg:hidden ${className}`} data-node-id="513:3059">
      {tabs.map((tab) => {
