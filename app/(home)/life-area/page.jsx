...
function ResultCard({ result, approved, onApprove }) {
  return (
    <article className="flex w-[328px] max-w-full flex-col gap-[19px] rounded-bl-[30px] rounded-br-[30px] rounded-tr-[30px] bg-[#d5d5d5] px-[22px] py-5">
      ...
      <div className="grid w-full grid-cols-3 gap-3.5">
        <button
          ...
          className={`flex h-[45px] w-full min-w-0 items-center justify-center rounded-[20px] px-2 text-sm text-white transition-colors ${approved ? "bg-[#5f6065] font-semibold ring-2 ring-inset ring-[#28292e]/20" : "bg-[#a8a8a8] hover:bg-[#929292]"}`}
          onClick={onApprove}
          type="button"
        >
          {approved ? "승인됨" : "승인"}
        </button>
        <button className="h-[45px] w-full min-w-0 rounded-[20px] bg-[#a8a8a8] px-2 text-sm text-white hover:bg-[#929292]" type="button">수정</button>
        <button className="h-[45px] w-full min-w-0 rounded-[20px] bg-[#a8a8a8] px-2 text-sm text-white hover:bg-[#929292]" type="button">삭제</button>
      </div>
    </article>
  );
}
...
export default function LifeAreaPage() {
  ...
  return (
    <PageContainer className="gap-[22px] pb-[118px] pt-[70px]" data-node-id="439:1396">
      <PageHeader title="계획 작성" backHref="/plan" backLabel="직전 화면으로 돌아가기" history className="pb-5" />

      <div className="self-start rounded-bl-[30px] rounded-br-[30px] rounded-tr-[30px] bg-[#d5d5d5] px-[22px] py-[18px] text-sm font-semibold">
        대화하듯 편하게 계획을 말씀해 주세요.
      </div>

      <div className="max-w-[290px] self-end rounded-bl-[30px] rounded-br-[30px] rounded-tl-[30px] bg-white px-[22px] py-[18px] text-sm leading-6">
        가족에게 가족사진 위치를 알려주고, 업무 정리 담당자에게 디자인 프로젝트를 인계한 뒤, 관계 정리 담당자에게 SNS와 클라우드 정리를 부탁하고 싶어요.
      </div>

      <div className="self-start rounded-bl-[30px] rounded-br-[30px] rounded-tr-[30px] bg-[#d5d5d5] px-[22px] py-[18px] text-sm font-semibold leading-6">
      ...
      <Button href="/manager">역할별 담당자 등록하기</Button>

      <div className="fixed inset-x-0 bottom-0 z-20 mx-auto w-full max-w-[390px] bg-gradient-to-t from-[#f0f0f2] via-[#f0f0f2] to-transparent px-6 pb-[max(24px,env(safe-area-inset-bottom))] pt-6">
