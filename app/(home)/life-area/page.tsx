...
import { getLifeAreaResults } from "@/lib/api";

type LifeAreaResult = ReturnType<typeof getLifeAreaResults>[number];

function ResultCard({ result, approved, onApprove }: { result: LifeAreaResult; approved: boolean; onApprove: () => void }) {
  return (
    <article className="flex h-full w-[328px] max-w-full min-w-0 flex-col gap-[19px] rounded-bl-[30px] rounded-br-[30px] rounded-tr-[30px] bg-[#d5d5d5] px-[22px] py-5 md:w-full md:max-w-none md:gap-3 md:px-[30px] md:py-5">
      ...
      <div className="grid w-full grid-cols-3 gap-3.5 md:gap-3">
        ...
        <button
          className="h-[45px] w-full min-w-0 whitespace-nowrap rounded-[20px] bg-[#a8a8a8] px-2 text-sm text-white hover:bg-[#929292] md:h-10"
          type="button"
        >
          수정
        </button>
...
export default function LifeAreaPage() {
  const results = getLifeAreaResults();
  ...
  return (
    <PageContainer
      className="gap-[22px] scroll-pb-[240px] pb-0 pt-[70px] md:gap-7 md:scroll-pb-[280px] md:pb-0"
      data-node-id="439:1396"
    >
      <PageHeader
        title="계획 작성"
        backHref="/plan"
        backLabel="직전 화면으로 돌아가기"
        history
        className="pb-5"
      />
      ...
      <div className="max-w-[290px] self-end rounded-bl-[30px] rounded-br-[30px] rounded-tl-[30px] bg-white px-[22px] py-[18px] text-sm leading-6 md:max-w-[65%] md:px-[30px] md:py-5 md:text-base">
        가족에게 가족사진 위치를 알려주고, 업무 정리 담당자에게 디자인
        프로젝트를 인계한 뒤, 관계 정리 담당자에게 SNS와 클라우드 정리를
        부탁하고 싶어요.
      </div>
      ...
      <div className="w-full shrink-0">
        <Button className="md:h-[52px]" href="/manager">
          역할별 담당자 등록하기
        </Button>
      </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 mx-auto w-full max-w-[390px] bg-gradient-to-t from-[#f0f0f2] via-[#f0f0f2] to-transparent px-6 pb-[max(24px,env(safe-area-inset-bottom))] pt-6 md:left-[140px] md:right-[140px] md:w-auto md:max-w-none md:px-0 md:pb-[50px] md:pt-8">
        <form
          className="flex w-full items-center justify-between rounded-[30px] bg-white px-[22px] py-3.5 shadow-[0_4px_20px_rgba(40,41,46,0.06)]"
        ...
        >
          ...
          >
            <Image
              src="/icons/plan/paper-plane-tilt.svg"
              alt=""
              width={24}
              height={24}
            />
