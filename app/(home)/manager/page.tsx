  ...
  return (
    <fieldset className="flex w-full flex-col gap-[22px]">
      ...
      <OutlineButton
        ...
        className={
          hasBackup
            ? "border-[#838383] bg-white text-[#5f6065] hover:bg-[#e7e7e9]"
            : ""
        }
...
export default function ManagerPage() {
  ...
  const [periods, setPeriods] = useState<Record<string, number | null>>({
    family: null,
    relation: null,
    work: null,
  });
  ...
  const setPeriod = (roleId: string, days: number) => {
    setPeriods((current) => ({
      ...current,
  ...
  const toggleBackup = (roleId: string) => {
    setBackups((current) => ({
      ...current,
        ...
        {roles.map((role, index) => (
          <div className="flex flex-col gap-[30px]" key={role.id}>
            <ManagerSection
              ...
              onPeriodChange={(days: number) => setPeriod(role.id, days)}
              role={role}
              waitingPeriod={periods[role.id]}
            />
            {index < roles.length - 1 && (
              <hr className="border-0 border-t border-[#d0d0d2]" />
            )}
          </div>
        ))}
